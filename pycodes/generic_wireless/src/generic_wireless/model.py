...
class GenericWirelessProcess:
    ...
    def init(self, builder: ms.ProcessBuilder) -> None:
        ...
        builder.add_state("Register", self.enter_register, self.exit_register)
        builder.add_state("Obtain", self.enter_obtain, self.exit_obtain)
        builder.add_state("RefreshRx", self.enter_refresh_rx, self.exit_refresh_rx)
        builder.add_state("Idle", self.enter_idle, self.exit_idle)
        builder.add_state("Disabled", self.enter_disabled, self.exit_disabled)

        builder.add_transition("Init", "Register", lambda: self.working_status)
        builder.add_transition("Init", "Disabled", lambda: not self.working_status)
        builder.add_transition("Register", "Obtain", _always_true)
        builder.add_transition("Obtain", "RefreshRx", _always_true)
        builder.add_transition("RefreshRx", "Idle", _always_true)
        builder.add_transition("Idle", "Idle", _always_true)
        builder.add_transition("Disabled", "Disabled", _always_true)
